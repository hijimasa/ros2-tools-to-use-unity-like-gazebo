...
{
    public class TransferFromRosHandlerMinimum
    {
        ...
        public TransferFromRosHandlerMinimum()
        {
        ...
        public bool GenerateModelIfReady()
        {
...
#if ROS2
            string[] urdfNames = urdfParameter.Split(':');
            string urdfName = urdfNames[1];
#else
            string urdfName = urdfParameter;
#endif
            Urdf.Editor.UrdfRobotExtensions.Create(Path.Combine(
                localDirectory,
                Path.GetFileNameWithoutExtension(urdfName) + ".urdf"));
